...
extern "C" void data_float_to_out(float * ptr, long width, long height, long depth, char * name);
	...
	float *filterSmooth, float *derivate, int t, float *gx, float*gy, float *gz)
{
	...
	cudaMemcpy(gx, gx_cuda, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(gy, gy_cuda, size * sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(gz, gz_cuda, size * sizeof(float), cudaMemcpyDeviceToHost);
